...
function checkDuplicates(name,email,company,leads,excludeId){
  ...
  leads.forEach(l=>{
    ...
    let score=0;
    if(n&&(l.name||"").toLowerCase().trim()===n)score+=3;
    else if(n&&(l.name||"").toLowerCase().includes(n))score+=1;
    if(e&&e===(l.email||"").toLowerCase().trim())score+=4;
    if(c&&c===(l.company||"").toLowerCase().trim())score+=1;
    if(score>=3)dupes.push({lead:l,score});
  });
...
function CommentsSection({leadId,user,profiles}){
  ...
  return(<div style={{marginTop:20}}>
    ...
    <div style={{display:"flex",gap:8,marginBottom:12}}>
      <input value={text} onChange={e=>setText(e.target.value)} placeholder="Add a comment..." style={{flex:1,padding:"7px 10px",borderRadius:8,border:"1px solid #e2e8f0",fontSize:13}} onKeyDown={e=>e.key==="Enter"&&add()}/>
      <Btn sz="sm" onClick={add}>Post</Btn>
    </div>
    {comments.map(c=><div key={c.id} style={{padding:"8px 0",borderBottom:"1px solid #f8fafc"}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}><span style={{fontSize:12,fontWeight:600,color:"#6366f1"}}>{getName(c.user_id)}</span><span style={{fontSize:11,color:"#94a3b8"}}>{fmtDateTime(c.created_at)}</span></div>
      <div style={{fontSize:13,color:"#475569"}}>{c.details?.text}</div>
    </div>)}
    {comments.length===0&&<div style={{fontSize:12,color:"#cbd5e1",textAlign:"center",padding:8}}>No comments yet</div>}
  </div>);
}
...
function ActivityTimeline({leadId,profiles}){
  ...
  return(<div style={{marginTop:20}}>
    <div style={{fontSize:13,fontWeight:700,color:"#334155",marginBottom:10,display:"flex",alignItems:"center",gap:6}}>{IC.clock} Activity Timeline</div>
    {acts.map(a=><div key={a.id} style={{display:"flex",gap:10,padding:"6px 0",borderBottom:"1px solid #f8fafc"}}>
      <div style={{width:8,height:8,borderRadius:99,background:"#6366f1",marginTop:5,flexShrink:0}}/>
      <div><div style={{fontSize:13}}><span style={{fontWeight:600,color:"#334155"}}>{getName(a.user_id)}</span>{" "}<span style={{color:"#64748b"}}>{desc(a)}</span></div><div style={{fontSize:11,color:"#94a3b8"}}>{fmtDateTime(a.created_at)}</div></div>
    </div>)}
    {acts.length===0&&<div style={{fontSize:12,color:"#cbd5e1",textAlign:"center",padding:8}}>No activity yet</div>}
  </div>);
}
...
function FileAttachments({leadId,user}){
  ...
  return(<div style={{marginTop:20}}>
    <div style={{fontSize:13,fontWeight:700,color:"#334155",marginBottom:10,display:"flex",alignItems:"center",gap:6}}>{IC.clip} Attachments</div>
    ...
    <Btn v="secondary" sz="sm" onClick={()=>ref.current?.click()} style={{marginTop:8}} disabled={uploading}>{IC.clip} {uploading?"Uploading...":"Attach File"}</Btn>
  </div>);
}
  ...
  return(
    <div style={{position:"fixed",right:0,top:0,bottom:0,width:460,maxWidth:"100vw",background:"#fff",boxShadow:"-10px 0 40px rgba(0,0,0,.1)",zIndex:999,overflow:"auto",borderLeft:"1px solid #e2e8f0",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{display:"flex",gap:0,borderBottom:"1px solid #f1f5f9",flexShrink:0}}>
        {tabs.map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"10px 16px",fontSize:12,fontWeight:600,border:"none",background:"none",cursor:"pointer",color:tab===t.id?"#6366f1":"#94a3b8",borderBottom:tab===t.id?"2px solid #6366f1":"2px solid transparent"}}>{t.label}</button>)}
      </div>
...
function LeadsTable({leads,stages,profiles,customFields,dispatch}){
  ...
  const getCellContent=(col,l)=>{
    ...
    switch(col.id){
      ...
      case "stage":return <Badge color={st?.color||"#6366f1"}>{st?.name||"—"}</Badge>;
      case "value":return <span style={{fontWeight:600}}>{fmt$(l.value)}</span>;
      case "assignee":return <span style={{color:"#64748b",fontSize:13}}>{as?.full_name||"—"}</span>;
      case "source":return <span style={{color:"#64748b"}}>{l.source||"—"}</span>;
      case "email":return <span style={{color:"#64748b",fontSize:13}}>{l.email||"—"}</span>;
      case "phone":return <span style={{color:"#64748b",fontSize:13}}>{l.phone||"—"}</span>;
      case "created":return <span style={{color:"#94a3b8",fontSize:13}}>{l.created_at?fmtDate(l.created_at):"—"}</span>;
      default:
        if(col.cfId)return <span style={{color:"#475569",fontSize:13}}>{l.custom_data?.[col.cfId]||"—"}</span>;
        return "—";
    }
  };
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"flex-end",marginBottom:8,position:"relative"}}>
      <Btn v="ghost" sz="sm" onClick={()=>setShowColPicker(!showColPicker)}>{IC.gear} Columns</Btn>
      {showColPicker&&<div style={{position:"absolute",top:36,right:0,background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:12,boxShadow:"0 8px 24px rgba(0,0,0,0.12)",zIndex:50,width:220}}>
        <div style={{fontSize:12,fontWeight:700,color:"#64748b",marginBottom:8}}>Show/Hide Columns</div>
        {cols.map(c=><label key={c.id} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 4px",cursor:c.system?"default":"pointer",fontSize:13,color:c.system?"#94a3b8":"#334155"}}>
          <input type="checkbox" checked={c.on} onChange={()=>toggleCol(c.id)} disabled={c.system} style={{accentColor:"#6366f1"}}/>
          {c.label}{c.system&&<span style={{fontSize:10,color:"#cbd5e1"}}>(required)</span>}
        </label>)}
        <div style={{borderTop:"1px solid #f1f5f9",marginTop:8,paddingTop:8}}>
          <Btn v="ghost" sz="sm" onClick={()=>setShowColPicker(false)}>Done</Btn>
        </div>
      </div>}
    </div>
...
function CalendarView({leads}){
  ...
  for(let i=1;i<=daysInMonth;i++)days.push(new Date(year,mo,i));
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      <Btn v="secondary" sz="sm" onClick={prev}>← Prev</Btn>
      <h3 style={{fontSize:16,fontWeight:700}}>{month.toLocaleDateString("en-US",{month:"long",year:"numeric"})}</h3>
      <Btn v="secondary" sz="sm" onClick={next}>Next →</Btn>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:1,background:"#e2e8f0",borderRadius:12,overflow:"hidden"}}>
      {["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(d=><div key={d} style={{padding:"8px",textAlign:"center",fontSize:12,fontWeight:600,color:"#64748b",background:"#f8fafc"}}>{d}</div>)}
      {days.map((day,i)=>{
...
export default function CRMDashboard(){
  ...
  const saveLead=async(f)=>{
    if(f.id&&leads.find(l=>l.id===f.id)){
      ...
      if(data){dispatch({type:"UPD_LEAD",p:data});await logActivity(data.id,user.id,"updated",{});}
    }else{
      const{id,...rest}=f;
      const{data}=await supabase.from("leads").insert({...rest,created_by:user.id}).select().single();
      if(data){dispatch({type:"ADD_LEAD",p:data});await logActivity(data.id,user.id,"created",{});}
    }
  ...
  const filtered=useMemo(()=>{
    ...
    if(filters.assignee)r=r.filter(l=>l.assignee_id===filters.assignee);
    return r;
  ...
  return(
    <div style={{display:"flex",height:"100vh",fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif",background:"#f8fafc",color:"#1e293b",overflow:"hidden"}}>
      ...
      {modal?.type==="addLead"&&<Modal title="New Lead" onClose={()=>dispatch({type:"SET_MODAL",p:null})}><LeadForm stages={stages} customFields={customFields} profiles={profiles} leads={leads} onSave={saveLead} onClose={()=>dispatch({type:"SET_MODAL",p:null})}/></Modal>}
      {modal?.type==="editLead"&&<Modal title="Edit Lead" onClose={()=>dispatch({type:"SET_MODAL",p:null})}><LeadForm lead={modal.lead} stages={stages} customFields={customFields} profiles={profiles} leads={leads} onSave={saveLead} onClose={()=>dispatch({type:"SET_MODAL",p:null})}/></Modal>}
    </div>
  );
}
