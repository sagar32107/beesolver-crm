...
const IC = {
  leads: <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>,
  dash: <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>,
  pipe: <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>,
  tasks: <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>,
  ...
  logout: <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4M16 17l5-5-5-5M21 12H9"/></svg>,
};
...
function Badge({ color, children, onClick, style: xs }) {
  return <span onClick={onClick} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 10px", borderRadius: 999, fontSize: 12, fontWeight: 600, background: color + "18", color, cursor: onClick ? "pointer" : "default", whiteSpace: "nowrap", ...xs }}>{children}</span>;
}
...
function Btn({ children, v = "primary", sz = "md", onClick, style: xs, disabled }) {
  ...
  const vs = {
    primary: { background: "#6366f1", color: "#fff", padding: sz === "sm" ? "6px 12px" : "8px 18px", fontSize: sz === "sm" ? 12 : 14 },
    secondary: { background: "#f1f5f9", color: "#334155", padding: sz === "sm" ? "6px 12px" : "8px 18px", fontSize: sz === "sm" ? 12 : 14 },
    ghost: { background: "transparent", color: "#64748b", padding: "6px 8px", fontSize: 13 },
    danger: { background: "#fee2e2", color: "#ef4444", padding: sz === "sm" ? "6px 12px" : "8px 18px", fontSize: sz === "sm" ? 12 : 14 },
  };
...
function Inp({ label, value, onChange, type = "text", placeholder, options }) {
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      {type === "select" ? <select value={value} onChange={e => onChange(e.target.value)} style={s}><option value="">Select...</option>{(options || []).map(o => <option key={o} value={o}>{o}</option>)}</select>
...
function Modal({ title, onClose, children }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 520, maxHeight: "90vh", overflow: "auto", boxShadow: "0 25px 50px -12px rgba(0,0,0,.25)" }}>
...
function LeadForm({ lead, stages, customFields, profiles, onSave, onClose }) {
  ...
  return (
    <>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 16px" }}>
        ...
        <Inp label="Stage" value={f.stage_id} onChange={v => u("stage_id", v)} type="select" options={stages.map(s => s.id)} />
        ...
        <Inp label="Assignee" value={f.assignee_id} onChange={v => u("assignee_id", v)} type="select" options={profiles.map(p => p.id)} />
      </div>
      {customFields.length > 0 && (<>
        <div style={{ fontSize: 12, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 1, margin: "8px 0" }}>Custom Fields</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 16px" }}>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 8 }}>
        ...
        <Btn onClick={() => { if (!f.name) return; onSave({ ...f, value: Number(f.value) || 0 }); }}>{lead ? "Update" : "Create Lead"}</Btn>
      </div>
    </>
  );
}
...
function LeadDetail({ lead, stages, customFields, profiles, dispatch, onClose, user }) {
  ...
  useEffect(() => {
    supabase.from("tasks").select("*").eq("lead_id", lead.id).order("created_at").then(({ data }) => setTasks(data || []));
  }, [lead.id]);
  ...
  return (
    <div style={{ position: "fixed", right: 0, top: 0, bottom: 0, width: 420, maxWidth: "100vw", background: "#fff", boxShadow: "-10px 0 40px rgba(0,0,0,.1)", zIndex: 999, overflow: "auto", borderLeft: "1px solid #e2e8f0" }}>
      ...
      <div style={{ padding: 20 }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
          <Badge color={stage?.color || "#6366f1"}>{stage?.name || "—"}</Badge>
          <Badge color="#64748b">{fmt$(lead.value)}</Badge>
        ...
        <div style={{ marginTop: 24 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#334155", marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}>{IC.tasks} Tasks</div>
          {tasks.map(t => (
        ...
        <div style={{ marginTop: 24, display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {stages.filter(s => s.id !== lead.stage_id).map(s =>
            <Badge key={s.id} color={s.color} onClick={() => moveStage(s.id)} style={{ cursor: "pointer", fontSize: 11 }}>{s.name}</Badge>
          )}
        </div>
      </div>
    </div>
...
function LeadsTable({ leads, stages, dispatch }) {
  return (
    <div style={{ overflowX: "auto", borderRadius: 12, border: "1px solid #e2e8f0" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
        <thead><tr style={{ background: "#f8fafc" }}>
          {["Name", "Company", "Stage", "Value", "Source", "Created"].map(h =>
            <th key={h} style={{ textAlign: "left", padding: "10px 14px", fontWeight: 600, color: "#64748b", fontSize: 12, textTransform: "uppercase", letterSpacing: .5 }}>{h}</th>
          )}
        </tr></thead>
        <tbody>
          {leads.map(l => { const st = stages.find(s => s.id === l.stage_id); return (
            <tr key={l.id} onClick={() => dispatch({ type: "SET_SEL", p: l })} style={{ cursor: "pointer", borderTop: "1px solid #f1f5f9" }} onMouseEnter={e => e.currentTarget.style.background = "#f8fafc"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
              <td style={{ padding: "10px 14px" }}><div style={{ fontWeight: 600 }}>{l.name}</div><div style={{ fontSize: 12, color: "#94a3b8" }}>{l.email}</div></td>
              <td style={{ padding: "10px 14px", color: "#475569" }}>{l.company}</td>
              <td style={{ padding: "10px 14px" }}><Badge color={st?.color || "#6366f1"}>{st?.name || "—"}</Badge></td>
              <td style={{ padding: "10px 14px", fontWeight: 600 }}>{fmt$(l.value)}</td>
              <td style={{ padding: "10px 14px", color: "#64748b" }}>{l.source}</td>
              <td style={{ padding: "10px 14px", color: "#94a3b8", fontSize: 13 }}>{l.created_at ? fmtDate(l.created_at) : "—"}</td>
            </tr>
          ); })}
          {leads.length === 0 && <tr><td colSpan={6} style={{ textAlign: "center", padding: 40, color: "#94a3b8" }}>No leads found.</td></tr>}
        </tbody>
      </table>
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 12 }}>
      {stages.filter(s => s.name !== "Lost").map(stage => {
        ...
        return (
          <div key={stage.id} style={{ minWidth: 240, flex: "1 0 240px", background: "#f8fafc", borderRadius: 12, padding: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 10, height: 10, borderRadius: 99, background: stage.color }} /><span style={{ fontWeight: 700, fontSize: 13 }}>{stage.name}</span><span style={{ background: "#e2e8f0", borderRadius: 99, padding: "1px 8px", fontSize: 11, fontWeight: 600 }}>{sl.length}</span></div>
              <span style={{ fontSize: 12, color: "#64748b", fontWeight: 600 }}>{fmt$(tot)}</span>
            </div>
            {sl.map(l => (
              <div key={l.id} onClick={() => dispatch({ type: "SET_SEL", p: l })} style={{ background: "#fff", borderRadius: 10, padding: 12, cursor: "pointer", border: "1px solid #e2e8f0", marginBottom: 8 }} onMouseEnter={e => e.currentTarget.style.boxShadow = "0 2px 8px rgba(0,0,0,.08)"} onMouseLeave={e => e.currentTarget.style.boxShadow = "none"}>
...
function DashboardView({ leads, stages }) {
  ...
  const cards = [
    ...
    { label: "Pipeline Value", value: fmt$(totalVal), color: "#3b82f6" },
    { label: "Won Revenue", value: fmt$(wonVal), color: "#10b981" },
    { label: "Conversion", value: conv + "%", color: "#f59e0b" },
  ];
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 12, padding: 16, border: "1px solid #e2e8f0" }}>
          ...
          {srcEntries.map(([src, cnt]) => (
            <div key={src} style={{ marginBottom: 10 }}><div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 3 }}><span style={{ fontWeight: 600 }}>{src}</span><span style={{ color: "#64748b" }}>{cnt}</span></div><div style={{ height: 8, background: "#f1f5f9", borderRadius: 99 }}><div style={{ height: "100%", borderRadius: 99, background: "#6366f1", width: ((cnt / maxSrc) * 100) + "%" }} /></div></div>
          ))}
        </div>
      </div>
    </div>
...
function SettingsView({ stages, customFields, dispatch, profile }) {
  ...
  const addStage = async () => {
    if (!ns.name || !isAdmin) return;
    const { data } = await supabase.from("stages").insert({ name: ns.name, color: ns.color, position: stages.length }).select().single();
    if (data) dispatch({ type: "SET_STAGES", p: [...stages, data] });
    setNS({ name: "", color: "#6366f1" });
  };
  const delStage = async (id) => {
    const st = stages.find(s => s.id === id);
    if (st?.is_system || !isAdmin) return;
    await supabase.from("stages").delete().eq("id", id);
    dispatch({ type: "SET_STAGES", p: stages.filter(s => s.id !== id) });
  };
  const addField = async () => {
    if (!nf.name || !isAdmin) return;
    const opts = nf.type === "select" ? nf.options.split(",").map(o => o.trim()).filter(Boolean) : [];
    const { data } = await supabase.from("custom_fields").insert({ name: nf.name, field_type: nf.type, options: opts, position: customFields.length }).select().single();
    if (data) dispatch({ type: "SET_CF", p: [...customFields, data] });
    setNF({ name: "", type: "text", options: "" });
  };
  const delField = async (id) => {
    if (!isAdmin) return;
    await supabase.from("custom_fields").delete().eq("id", id);
    dispatch({ type: "SET_CF", p: customFields.filter(f => f.id !== id) });
  };
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "flex-end", flexWrap: "wrap" }}>
        ...
        {nf.type === "select" && <Inp label="Options (comma)" value={nf.options} onChange={v => setNF(p => ({ ...p, options: v }))} placeholder="Low, Medium, High" />}
        <Btn onClick={addField} style={{ marginBottom: 12 }}>{IC.plus} Add</Btn>
      </div>
    </div>
  );
}
...
export default function CRMDashboard() {
  ...
  const saveLead = async (f) => {
    if (f.id && leads.find(l => l.id === f.id)) {
      const { data } = await supabase.from("leads").update(f).eq("id", f.id).select().single();
      if (data) dispatch({ type: "UPD_LEAD", p: data });
    } else {
      const { id, ...rest } = f;
      const { data } = await supabase.from("leads").insert({ ...rest, created_by: user.id }).select().single();
      if (data) dispatch({ type: "ADD_LEAD", p: data });
    }
  ...
  const filtered = useMemo(() => {
    let r = leads;
    if (search) { const s = search.toLowerCase(); r = r.filter(l => (l.name || "").toLowerCase().includes(s) || (l.company || "").toLowerCase().includes(s) || (l.email || "").toLowerCase().includes(s)); }
    if (filters.stage) r = r.filter(l => l.stage_id === filters.stage);
  ...
  const nav = [
    ...
    { id: "leads", icon: IC.leads, label: "Leads" },
    { id: "pipeline", icon: IC.pipe, label: "Pipeline" },
    ...
    <div style={{ display: "flex", height: "100vh", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", background: "#f8fafc", color: "#1e293b", overflow: "hidden" }}>
      <div style={{ width: 220, background: "#fff", borderRight: "1px solid #e2e8f0", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        ...
        <div style={{ padding: 14, borderTop: "1px solid #f1f5f9" }}>
          ...
          <button onClick={logout} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", color: "#94a3b8", fontSize: 12, fontWeight: 600, padding: 0 }}>{IC.logout} Sign out</button>
        </div>
      </div>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: "12px 20px", background: "#fff", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexShrink: 0 }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800 }}>{page === "dashboard" ? "Dashboard" : page === "leads" ? "Leads" : page === "pipeline" ? "Pipeline" : "Settings"}</h2>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: 20 }}>
          ...
          {page === "leads" && (view === "table" ? <LeadsTable leads={filtered} stages={stages} dispatch={dispatch} /> : <PipelineView leads={filtered} stages={stages} dispatch={dispatch} />)}
          {page === "pipeline" && <PipelineView leads={filtered} stages={stages} dispatch={dispatch} />}
          {page === "settings" && <SettingsView stages={stages} customFields={customFields} dispatch={dispatch} profile={profile} />}
        </div>
      </div>
      {selLead && <LeadDetail lead={selLead} stages={stages} customFields={customFields} profiles={profiles} dispatch={dispatch} onClose={() => dispatch({ type: "SET_SEL", p: null })} user={user} />}
      {modal?.type === "addLead" && <Modal title="New Lead" onClose={() => dispatch({ type: "SET_MODAL", p: null })}><LeadForm stages={stages} customFields={customFields} profiles={profiles} onSave={saveLead} onClose={() => dispatch({ type: "SET_MODAL", p: null })} /></Modal>}
      {modal?.type === "editLead" && <Modal title="Edit Lead" onClose={() => dispatch({ type: "SET_MODAL", p: null })}><LeadForm lead={modal.lead} stages={stages} customFields={customFields} profiles={profiles} onSave={saveLead} onClose={() => dispatch({ type: "SET_MODAL", p: null })} /></Modal>}
    </div>
  );
}
